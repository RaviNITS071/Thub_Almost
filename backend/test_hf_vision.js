...
async function testHuggingFaceVision() {
  console.log('--- Hugging Face Vision Configuration Check ---');
  const token = process.env.HF_TOKEN || '';
  const model = process.env.HF_MODEL || 'zai-org/GLM-4.5V';
  console.log('HF_TOKEN:', token ? token.slice(0, 7) + '******' + token.slice(-4) : '(not set)');
  console.log('HF_MODEL:', model);

  if (!token || token.includes('your_huggingface_token')) {
    console.error('\n❌ Missing or dummy HF_TOKEN in backend/.env');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`\nSending image to Hugging Face router (${model})...`);
    const startTime = Date.now();
    const solution = await captchaService.solveWithHuggingFace(imgBuffer.toString('base64'));
    const durationMs = Date.now() - startTime;

    console.log('\n========================================');
    console.log('🎉 HUGGING FACE VISION RESULT:');
    ...
    if (!stillCaptcha || hasTenderDetails) {
      console.log('======================================================');
      console.log(`✅ PORTAL ACCEPTED! Hugging Face Vision passed on first attempt!`);
      console.log('======================================================\n');
    } else {
